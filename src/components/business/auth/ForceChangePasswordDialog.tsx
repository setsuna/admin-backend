...
export const ForceChangePasswordDialog: React.FC<ForceChangePasswordDialogProps> = ({
  open,
  onClose,
  onSuccess,
  userId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await auth.forceChangePassword({
        user_id: userId,
    ...
    >
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="flex items-start gap-3 p-3 bg-warning/10 border border-warning/30 rounded-lg">
          <KeyRound className="h-5 w-5 text-warning flex-shrink-0 mt-0.5" />
          <div className="text-sm">
            <p className="font-medium text-warning">密码已过期</p>
            <p className="text-text-secondary mt-1">
              用户 <span className="font-medium text-text-primary">{username}</span> 的密码已过期，请设置新密码后继续使用系统。
            </p>
          </div>
        </div>
