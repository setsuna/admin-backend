...
import { Search, RefreshCw, Shield, Users, ShieldCheck, ShieldAlert, ShieldX, Key } from 'lucide-react'
...
const SecurityLevelManagePage = () => {
  ...
  const handleSecurityLevelSubmit = async (formData: { securityLevel: UserSecurityLevel; reason?: string }) => {
    ...
    const levelMap = { unknown: '未知', internal: '内部', confidential: '秘密', secret: '机密' }
    ...
    if (confirmed) {
      updateUserSecurityLevel({ id: editingUser.id, securityLevel: formData.securityLevel })
  ...
  const handleBatchUpdateSubmit = async (formData: { securityLevel: UserSecurityLevel; reason?: string }) => {
    ...
    if (confirmed) {
      batchUpdateSecurityLevel({ ids: selectedIds, securityLevel: formData.securityLevel })
    ...
    },
    {
      ...
      render: (securityLevel: UserSecurityLevel) => {
        const levelMap: Record<UserSecurityLevel, { label: string; color: string; icon: any }> = {
          ...
          confidential: { label: '秘密', color: 'yellow', icon: ShieldAlert },
          secret: { label: '机密', color: 'red', icon: ShieldCheck }
        }
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500">秘密级别</p>
