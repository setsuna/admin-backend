...
import { Plus, Edit, Trash2, Search, RefreshCw, Building2, Users, Phone, Mail, MapPin } from 'lucide-react'
...
const DepartmentPage = () => {
  ...
  // 查询部门列表
  const { data: departmentData, isLoading, error, refetch } = useQuery({
  ...
  const batchDeleteMutation = useMutation({
    ...
    onSuccess: () => {
      addNotification({ type: 'success', title: '批量删除成功', message: `已删除 ${selectedIds.length} 个部门` })
      setSelectedIds([])
      queryClient.invalidateQueries({ queryKey: ['departments'] })
      queryClient.invalidateQueries({ queryKey: ['departmentTree'] })
      queryClient.invalidateQueries({ queryKey: ['departmentOptions'] })
  ...
  const formFields = [
    ...
    },
    {
      ...
      options: [
        { label: '无上级部门', value: '' },
        ...(departmentOptions?.data || []).map(dept => ({
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-muted-foreground">
          {viewMode === 'table' && departmentData?.data.pagination && (
            <>共 {departmentData.data.pagination.total} 条记录</>
          )}
          {viewMode === 'tree' && departmentTree?.data && (
            <>共 {countTotalDepartments(departmentTree.data)} 个部门</>
          )}
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <Table
              data={departmentData?.data.items || []}
              columns={columns}
              loading={isLoading}
              pagination={departmentData?.data.pagination}
            ...
            <div className="p-6">
              <DepartmentTree
                ...
                onAdd={handleAdd}
                loading={departmentTree ? false : true}
              />
            </div>
          )}
        </CardContent>
